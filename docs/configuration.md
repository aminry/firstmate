...
The optional local, gitignored `config/wedge-defer-pipeline` presence flag opts this home into a default-off fourth form of positive work evidence in watcher triage.
...
With the flag absent nothing this behavior adds is observable: no deferral is taken, no marker is published, no new state file is written, and a stale pane escalates on exactly the schedule it did before.
The pre-existing worktree-write deferral is outside the flag and behaves the same whether or not it is set.
