...
The consult costs one status-line read, plus a status-log fold and then one current-state read for the lanes whose status line explained nothing, all taken in the same at-threshold branch as the worktree walk and never on an ordinary poll.
