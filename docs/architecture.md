...
A pane whose worktree is silent is then asked one further question, on the same at-threshold budget: does the validation pipeline itself report one of its own steps as currently producing output?
...
A positive answer takes the same bounded deferral as write evidence, on its own chain so each stretch ages from when that evidence started, and every other answer - a quiet step, no activity report, an unreadable verdict, an unreachable daemon, or any state that is not a working run step - leaves the escalation schedule exactly as it was.
