...
That flag is default-off and gates both halves, the deferral and the marker `bin/fm-crew-state.sh` publishes for it, so a home that has not set it takes no deferral, reads no marker, writes no new state file, and escalates on exactly the prior schedule.
