...
When the status line accounts for nothing, one further record is read: whether the crew's own current state is a validation gate whose answer is owed by a human.
...
It is minted only from the gate's own findings table, by a row whose `action` column is exactly `ask-user`, read by position out of the table header rather than searched for over the run payload, where a finding's free-text description or a branch name would satisfy a search just as well.
That precision is what keeps the distinction the ladder depends on: `awaiting_approval`, `fix_review` and `awaiting_agent` gates are all reported parked and are all answered by the crewmate whose run they belong to, and a crewmate that goes quiet before answering its own gate is exactly the wedge this ladder exists to catch, so those keep the unchanged escalation schedule, reason and `demand-deep-inspection` wording.
...
The consult costs one status-line read, plus one current-state read for the lanes whose status line explained nothing, both taken in the same at-threshold branch as the worktree walk and never on an ordinary poll.
