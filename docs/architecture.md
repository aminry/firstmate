...
When the status line accounts for nothing, one further record is read: whether the crew's own current state is a validation gate whose answer is owed by a human who has actually been asked and has not answered.
That record exists because the quiet of a parked lane is the pipeline's doing rather than anything the worker wrote down, so no status-line predicate can see it.
...
Its second half is the task's own decision fold still holding an open `needs-decision` record, which is the positive evidence that the human was told rather than merely named.
That half is what keeps the ladder in the two cases where a parked human-owed gate is really the crewmate's move: a captain who has already answered, where `fm-send --resolve-key` closed the decision at answer time while the gate stays parked until the crewmate relays it, and a crewmate that parked at such a gate and went quiet before escalating it at all, where nobody was ever told.
...
The two block on different people: a `paused:` declaration is owed by an external dependency the worker named and asks the reader to confirm the wait still holds, a hold is owed by the captain reading the recheck and asks them to answer the held decision or release the hold, and a parked gate is owed the captain's `ask-user` decision and asks for that finding to be answered.
...
While the away-posture record exists a captain-facing wait, whether a hold or a parked gate, is not rechecked here at all, as on every other captain-facing path: there is nobody to answer it and the return brief already lists it, so the pane is absorbed silently and no re-surface throttle is armed, leaving the recheck owed once the record is archived.
That absorb restarts the idle timer like every other deferral, so the recheck owed on return arrives within one `FM_STALE_ESCALATE_SECS` of the record being archived rather than instantly.
The consult costs one status-line read, plus a status-log fold and then one current-state read for the lanes whose status line explained nothing, all taken in the same at-threshold branch as the worktree walk, so it is bounded to at most once per window per `FM_STALE_ESCALATE_SECS` and never runs on an ordinary poll.
...
A crew whose own validation gate awaits a human decision that is still open is the second, reached through the shared wedge timer rather than the declaration branch, because who owes that answer does not depend on what the pane is rendering; it takes the same bounded recheck, and is likewise not rechecked while the away-posture record exists.
